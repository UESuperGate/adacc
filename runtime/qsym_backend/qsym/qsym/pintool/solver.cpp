...
Solver::Solver(
  ...
  , sub_solver_(z3::solver(context_, "QF_BV"))
  , num_generated_(0)
  , trace_(bitmap)
...
void Solver::sub_add(z3::expr expr) {
  if (!expr.is_const())
    sub_solver_.add(expr.simplify());
}
...
void Solver::sub_addToSolver(ExprRef e, bool taken) {
  e->simplify();
  if (!taken)
    e = g_expr_builder->createLNot(e);
  sub_add(e->toZ3Expr().simplify());
}

void Solver::dumpAsSTM2(ExprRef e, bool taken) {
  sub_reset();
  sub_addToSolver(e, !taken);
  
  ofstream myfile;
  myfile.open ("./old/" + toString6digit(num_generated_), ios::trunc);
  myfile << sub_solver_.to_smt2();
  myfile.close();
}
